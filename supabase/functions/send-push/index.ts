    ...
    try {
        ...
        const results = await Promise.all(subscriptions.map(async (sub) => {
            ...
            if (isFCMToken) {
                ...
                // It's an Expo token - would need Expo Push API
                console.log('Skipping Expo token (not supported in this version):', token.substring(0, 20));
                return { success: false, id: sub.id, type: 'expo', error: 'Expo tokens not supported' };
            }
        }));
